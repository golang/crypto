...
var argo2dtests = []struct {
	...
	{argon2d, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 0, []byte{}},
	{argon2d, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 32, []byte{
...
func testArgon2d(t *testing.T) {
	for _, tt := range argo2dtests {
		hash := deriveKey(tt.mode, tt.password, tt.salt, tt.secret, tt.data, tt.time, tt.memory, tt.threads, tt.keyLen)
		if !bytes.Equal(hash, tt.out) {
			t.Errorf("derived key does not match - got: %s , want: %s", hex.EncodeToString(hash), hex.EncodeToString(tt.out))
		}
	}
}

var argo2itests = []struct {
	mode     int
	password []byte
	salt     []byte
	secret   []byte
	...
	{argon2i, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 0, []byte{}},
	{argon2i, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 32, []byte{
...
var argo2idtests = []struct {
	...
	{argon2id, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 0, []byte{}},
	{argon2id, genKatPassword, genKatSalt, genKatSecret, genKatAAD, 3, 32, 4, 32, []byte{
