...
package ccm // import "golang.org/x/crypto/ccm"
...
// New returns the given 128-bit block cipher wrapped in CCM.
...
func New(b cipher.Block, tagsize, noncesize int) (CCM, error) {
	fmt.Println(tagsize, noncesize, 15-noncesize)
...
func (c *ccm) Open(dst, nonce, ciphertext, adata []byte) ([]byte, error) {
	...
	if subtle.ConstantTimeCompare(tag, expectedTag) != 1 {
		return nil, errors.New("ccm: tag missmatch")
	}
